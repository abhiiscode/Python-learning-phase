FILEPATH = '../rows.txt'
from day15_def_source_file import get_todos, write_todos
...
import time
real_time = time.strftime("%A - %d - %m(%b) - %Y %H:%M:%S")
print("it's a real time :- ", real_time)
...
while True:
  ...
  #this todos is not lical viariable its a globle variable
        todos = get_todos(filepath=FILEPATH)   #replace by➡️ #  with open('rows.txt', 'r') as file:
                                               #   todos = file.readlines()  #   method 2
        todos.append(todo + '\n')

        write_todos(filepath='../rows.txt', todos_arg=todos)   # todos is the list it's objects throws to local variable (todos_arg                            # with open('rows.txt', 'w') as file:
                                      #     file.writelines(todos)

    elif user_action.startswith('show'):
        todos = get_todos(filepath=FILEPATH) #Argument=value == filepath='rows.txt'
        ...
        for index, item in enumerate(todos):
           ...
           # print(index, item)
            ...
            write_todos(todos, filepath=FILEPATH)# with open('rows.txt', 'w') as file:
            #     file.writelines(todos)
        ...
        try:
            ...
            todos = get_todos(filepath=FILEPATH)
           ...
           # number = number + 1
            ...
            write_todos(todos, filepath=FILEPATH)  # it will posible in it: file.writelines(['a', 'b', 'c'])
        ...
        todos = get_todos(filepath=FILEPATH)
        print(len(todos))
        write_todos(filepath='../rows.txt', todos_arg=todos)
